  ...
  this.migration.all(function(err, appliedMigrations) {
    ...
    var migrationCalls = _.map(migrationModules, function(mod) {
      return function(done) {
        ...
        const migrationMethod = mod[direction];
        const migrationMethodExpectsCallback = migrationMethod.length > 0;
...
Migrator.prototype._up = function(migrationName, cb) {
  var self = this;
...
Migrator.prototype.up = callbackOrPromise(Migrator.prototype._up);

/**
 * down [name]
 */

Migrator.prototype._down =  function(migrationName, cb) {
  var self = this;
  this.setup(function(err) {
    if(err) return cb(err);
...
Migrator.prototype.down = callbackOrPromise(Migrator.prototype._down);

/**
 * create [title]
 */

Migrator.prototype._generate = function(title, cb){
  var self = this;
  this.mkdir(function(){
...
Migrator.prototype.generate = callbackOrPromise(Migrator.prototype._generate);

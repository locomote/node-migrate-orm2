...
const {callbackOrPromise} = require('./utils');
...
MigrationDSL.prototype = {
  ...
  _createTable: function (collectionName, options, cb) {
    var columns = [];
    var keys = [];
  ...
  _addColumn: function (collectionName, options, cb) {
...
callbackMethods.forEach(methodName => {
  MigrationDSL.prototype[methodName] = callbackOrPromise(MigrationDSL.prototype['_' + methodName]);
});
